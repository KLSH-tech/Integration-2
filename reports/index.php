...
$sectionLabels = array_map(fn($r) => $r['section'], $sectionCounts);
$sectionValues = array_map(fn($r) => (int)$r['count'], $sectionCounts);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="dashboard-wrap">
    ...
    <div class="hero-panel">
        <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
            ...
            <div class="hero-badge">MONDAY ATTENDANCE ONLY &middot; <?= date('F j, Y', strtotime($lastMonday)) ?></div>
        </div>
    </div>
    ...
    <div id="present-breakdown" class="breakdown-box">
        ...
        <?php if (empty($presentGrouped)): ?>
            <p class="no-data">No present records found.</p>
        <?php else: foreach ($presentGrouped as $year => $rows): if ((int)$year >= 1 && (int)$year <= 4): ?>
                ...
                <?php foreach ($rows as $row): ?>
                    <div class="col-md-3 mb-3">
                        <div class="mini-stat-card mini-present">
                            <div class="mini-code"><?= htmlspecialchars($row['subject_code']) ?></div>
    ...
    <div id="late-breakdown" class="breakdown-box">
        ...
        <?php if (empty($lateGrouped)): ?>
            <p class="no-data">No late records found.</p>
        <?php else: foreach ($lateGrouped as $year => $rows): if ((int)$year >= 1 && (int)$year <= 4): ?>
            <div class="mini-year-block">
                <div class="mini-year-title"><?= yearLabel($year) ?></div>
                <div class="row">
                <?php foreach ($rows as $row): ?>
    ...
    <div id="absent-breakdown" class="breakdown-box">
        ...
        <?php if (empty($absentGrouped)): ?>
            <p class="no-data">No absent records found.</p>
        <?php else: foreach ($absentGrouped as $year => $rows): if ((int)$year >= 1 && (int)$year <= 4): ?>
            <div class="mini-year-block">
                <div class="mini-year-title"><?= yearLabel($year) ?></div>
                <div class="row">
                <?php foreach ($rows as $row): ?>
    ...
    <div class="section-title">
        ...
        <span class="section-sub">Weekly attendance movement and section distribution</span>
    </div>
    ...
    <div class="analytics-shell mb-5">
            ...
            <div class="analytics-card-head">
                ...
                <div class="chart-stats">
                    <div class="mini-analytic-stat">
                        ...
                        <strong><?= (int)$mondayStats['present'] ?></strong>
                        <small>Present</small>
                    </div>
                    <div class="mini-analytic-stat">
                        <span class="legend-dot late"></span>
                        <strong><?= (int)$mondayStats['late'] ?></strong>
                        <small>Late</small>
                    </div>
                    <div class="mini-analytic-stat">
                        <span class="legend-dot absent"></span>
                        <strong><?= (int)$mondayStats['absent'] ?></strong>
                        <small>Absent</small>
                    </div>
                </div>
            </div>
        ...
        <div class="analytics-grid">
            <div class="analytics-card">
                <div class="analytics-card-head">
                    <div><h5>Monday Distribution</h5><p>Distribution of attendance status for the latest Monday</p></div>
                </div>
                <?php if ($mondayStats['total'] > 0): ?>
                    <div class="distribution-chart-wrap"><canvas id="mondayChart"></canvas></div>
                <?php else: ?>
                    <p class="text-center no-data mt-5">No attendance data yet.</p>
                <?php endif; ?>
            </div>
            <div class="analytics-card">
                <div class="analytics-card-head">
                    <div><h5>Students per Section</h5><p>Section population overview</p></div>
                </div>
                <div class="section-chart-wrap"><canvas id="sectionChart"></canvas></div>
            </div>
        </div>
    </div>
...
function hideAllBreakdowns() {
    ['present','late','absent'].forEach(t => {
        document.getElementById(t + '-breakdown')?.classList.remove('active');
    });
}
...
new Chart(document.getElementById('mondayChart'), {
    ...
    data: {
        ...
        datasets: [{
            data: [<?= (int)$mondayStats['present'] ?>, <?= (int)$mondayStats['late'] ?>, <?= (int)$mondayStats['absent'] ?>],
            backgroundColor: ['#1f3b73','#6db7ff','#e74a3b'],
            borderWidth: 0,
            hoverOffset: 6
        }]
    },
...
new Chart(document.getElementById('trendChart'), {
    ...
    options: {
        ...
        scales: {
            ...
            y: { beginAtZero:true, grid:{ color:'rgba(148,163,184,0.12)' }, ticks:{ color:'#9aa6b2', stepSize:1 } }
        }
    }
});
<?php endif; ?>
